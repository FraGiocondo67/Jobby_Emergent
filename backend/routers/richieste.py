...
async def create_richiesta(body: RichiestaIn, user=Depends(get_current_user)):
    ...
    if body.publish:
        ...
        # Auto-invito dei provider compatibili: la richiesta appare subito nelle loro "in arrivo".
    ...
    if body.publish:
        for inv in doc["provider_invitati"]:
            await push_notification(inv["provider_id"], "nuova_richiesta", "Nuova richiesta pulizie",
...
async def admin_invite(rid: str, body: InviteIn, _=Depends(require_admin)):
    ...
    for pid in body.provider_ids:
        if pid in already:
            # Re-assignment: if the provider had declined, reactivate their invite.
